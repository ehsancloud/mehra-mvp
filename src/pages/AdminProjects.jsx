...
import { getProjects, getCurrentUserId, getUserById } from "../data/api";
...
const AdminProjects = () => {
  ...
  const loadProjects = () => {
    if (activeTab === "archived") {
      ...
      Promise.all([
        getProjects({ stage: "completed", query: searchQuery }),
        getProjects({ stage: "archived", query: searchQuery }),
      ]).then(([completed, archived]) => setProjects([...completed, ...archived]));
    } else {
      getProjects({ stage: activeTab, query: searchQuery }).then(setProjects);
    }
  };

  useEffect(loadProjects, [activeTab, searchQuery]);
