...
import { getWallet, chargeWallet, getCurrentUserId } from "../data/api";
...
const EmployerWalletWidget = () => {
  ...
  useEffect(() => {
    getWallet(employerId).then((wallet) => setBalance(wallet.balance));
  }, [employerId]);

  // TODO(API): open a top-up flow / redirect to the payment gateway,
  // e.g. POST /wallet/charge, then redirect to the returned payment URL.
  const handleIncreaseCredit = async () => {
    const amount = prompt("مبلغ افزایش اعتبار (تومان) را وارد کنید:");
    if (!amount || Number.isNaN(Number(amount))) return;
    const result = await chargeWallet({ employerId, amount });
