...
const EmployerProjectDetailsModal = ({
  isOpen,
  onClose,
  project,
  onUpdated,
  ...
  // تابع درخواست اصلاحیه کارفرما
  const handleRequestEdit = async () => {
    const result = await editRequestByEmployer(project.id);
    if (!result.ok) {
      alert(result.message || "امکان ثبت درخواست اصلاحیه وجود ندارد.");
      return;
    }
    alert("درخواست اصلاحیه شما با موفقیت ثبت شد و پیام آن در تیکت درج گردید.");
    if (onUpdated) {
      const refreshed = await getProjectById(project.id);
      onUpdated(refreshed);
    }
  ...
  // تابع تایید نهایی خروجی و خاتمه پروژه
  const handleConfirmDeliverable = async () => {
    if (!window.confirm("آیا از تایید نهایی کار و بستن پروژه اطمینان دارید؟"))
      return;
    const result = await confirmResultResult(project.id);
    if (!result.ok) {
      alert(result.message || "خطا در تایید پروژه.");
      return;
    }
    alert("پروژه با موفقیت تایید و مختومه شد!");
    if (onUpdated) {
      const refreshed = await getProjectById(project.id);
      onUpdated(refreshed);
    }
        ...
        <div className="w-full flex gap-6 items-start mt-2">
              ...
              {project.stage === "active" && (
                <div className="flex gap-2 w-full mt-2">
                  <button
                    ...
                    className="flex-1 h-[36px] bg-[#fef3c7] text-[#92400e] border border-black rounded text-[12px] font-bold shadow-[0_2px_0_0_#000000] cursor-pointer"
                  >
                    درخواست اصلاحیه
                  </button>
                  <button
                    ...
                    className="flex-1 h-[36px] bg-[#def7ec] text-[#03543f] border border-black rounded text-[12px] font-bold shadow-[0_2px_0_0_#000000] cursor-pointer"
                  >
                    تایید نهایی خروجی
                  </button>
                </div>
          ...
          <div className="w-[230px] shrink-0 flex flex-col gap-4">
            ...
            {!project.isSuperProject && (
              <button
                onClick={() =>
                  openChat(project.ticketId || project.id, project.title)
                }
