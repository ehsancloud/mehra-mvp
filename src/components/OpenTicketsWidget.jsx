...
const OpenTicketsWidget = () => {
  ...
  useEffect(() => {
    if (user) {
      // ساخت پارامتر داینامیک بر اساس نقش کاربر
      const roleKey = user.role === "freelancer" ? "freelancerId" : user.role === "employer" ? "employerId" : "supervisorId";
      const params = { stage: "current" };
      if (user.role !== "admin") params[roleKey] = user.id;
      
      getTickets(params).then(setTickets);
    }
  }, [user]);
