...
const SupervisorTickets = () => {
  ...
  const supervisorId = getCurrentUserId("supervisor");
  const currentSupervisor = getUserById(supervisorId);

  useEffect(() => {
    getTickets({ supervisorId, stage: activeTab, query: searchQuery }).then(
      setTickets,
    );
  }, [supervisorId, activeTab, searchQuery]);
    ...
    >
      ...
      <div className="flex-1 w-full flex flex-col items-center overflow-hidden">
        ...
        >
          ...
          <button
            type="button"
            className="w-[100px] h-[38px] bg-white text-black border border-black rounded-[5px] shadow-[0_2px_0_0_#000000] active:translate-y-[1px] active:shadow-none text-[13px] font-bold cursor-pointer transition-all"
          >
            جستجو
          </button>
        </div>
